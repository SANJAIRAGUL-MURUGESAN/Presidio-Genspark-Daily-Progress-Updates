...
namespace EmployeeRequestTrackerAPI.Controllers
{
    ...
    public class EmployeeController : ControllerBase
    {
        ...
        public async Task<ActionResult<IList<Employee>>> Get()
        {
            ...
            } catch (NoSuchEmployeeException Nsfe)
            {
                return NotFound(new ErrorModel(404, Nsfe.Message));
            }
        }
        ...
        public async Task<ActionResult<EnableUserReturnDTO>> UpdateEmployeeStatus([FromBody] EnableUserDTO enableUserDTO)
        {
            try
            {
                var employee = await _employeeService.UpdateEmployeeStatus(enableUserDTO);
                return Ok(employee);
            }
            catch (NoSuchEmployeeException nefe)
            {
